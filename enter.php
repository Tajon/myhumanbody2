...
if (isset($_POST['name']) && isset($_POST['style'])) {
    ...
    $action = sanitizeMySQL($conn, $_POST['action']);
    $section = sanitizeMySQL($conn, $_POST['section']);
    $number = sanitizeMySQL($conn, $_POST['number']);


    // the prepared statement - note: 6 question marks represent
    // 6 variables we will send to database separately
    $query = "INSERT INTO socks (name, action, section, number)
    VALUES (?, ?, ?, ?, ?, ?)";
